...
### 最重要ルール
- **必ず `git status -s && git diff --cached` を実行し、ステージングされた変更内容を正確に把握すること。**
...
#### ケース1: 同一機能に対する複数の変更タイプ
**同じ機能領域に対して複数のタイプの変更がある場合は統合する**

例：
- `useSectionStatus.js`に新機能追加（feat）とリファクタリング（refactor）が混在
  → **統合**: `feat`として1つのコミットに
