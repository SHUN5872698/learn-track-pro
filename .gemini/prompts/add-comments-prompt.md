...
- メソッド名や変数名をそのまま繰り返しただけのWhat（何を）の説明
  - ❌ 悪い例: `async fetchContents() { // 学習内容を取得`
  - ✅ 良い例: `async fetchContents() { // 学習内容のリストを取得`（役割を説明）
- コードを読めば明らかな処理の流れ
...
- **グループ化ラベル**: 関連する処理をまとめ、複数のVueファイル間で構造を統一
  - 例: `// エラーをリセット`、`// 修正フラグをリセット`
  - 処理内容が明らかでも、ファイル間の構造統一のために使用
- **実行順序の注意**: 処理順序が重要な場合はNOTEで明示
  - 例: `// NOTE: API側エラーリセットは必ずボタン非活性化解除前に実行`
