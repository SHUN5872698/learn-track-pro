...
最初に以下のコマンドを実行し、**直近のコミットメッセージのスタイル（フォーマット、粒度、言語など）を必ず確認してください。**
...
Gitのステージングしているファイルのコミットメッセージについて、「キーワードの使い分けが適切か」の評価と解説を先に行い、その後で「より適切なコミットメッセージの添削例」を提示してください。
...
### 最重要ルール
- **必ず `git log` で直近のコミットメッセージを確認し、そのプロジェクトのスタイル（フォーマット、粒度、言語、絵文字の使用有無など）を完全に模倣すること。**
- Conventional Commitsの規約とプロジェクトのスタイルが異なる場合は、**プロジェクトのスタイルを優先する。**

### 評価手順
1.  **関心事の分離**: ステージングされたファイル群に複数の関心事（例: `feat`と`docs`）が混在している場合、それぞれを個別のコミットとして扱う。
2.  **キーワード（prefix）の妥当性評価**: 分離された関心事ごとに、キーワードがConventional Commitsガイドラインに適合しているか判定する。
    -   不適切な場合は、推奨するキーワードとその理由を説明する。
3.  **適切なコミットメッセージの提案**: 関心事ごとに、タイトルと必要に応じたbodyを含む完全なメッセージを提案する。**複数のコミットメッセージを提案する場合、1つのファイルにまとめて出力する。**
...
## 評価時の確認事項
1. **Conventional Commits仕様との適合性**を最優先で確認
2. **セマンティックバージョニングへの影響**を考慮
3. **変更の性質**（ユーザー影響、コード品質、プロジェクト管理）を正確に判
