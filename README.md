...
- **AgentSkillsによるコードレビュー自動化**
  - Antigravityの`.agents/skills/`機能を活用し、プロジェクト固有のコーディング規約に基づくレビューSkillを構築
  - 「レビューして」と入力するだけで、Laravel/Vue.jsの規約チェック・指摘分類（Critical/Warning/Suggestion）を自動実行
  - 既存コードでの精度検証・改善サイクルを2回実施し、実用レベルまで調整
...
- [ ] CI/CD パイプライン構築（GitHub Actions）
  - 品質を担保した継続的開発の基盤構築
  - ✅ PR単位のCI起動（PHPUnit）を導入済み
  - 残: 全テスト対象への拡張、ブランチ保護、CD（自動デプロイ）
- [ ] 自動テスト拡充
  - CI/CD実践に必要な品質保証基盤の整備
  - ✅ プロフィール画像アップロード機能のユニットテスト（8ケース）を作成済み
  - 残: 他機能のテスト作成

### 優先度: 中

- [x]  プロフィール画像アップロード機能
  - 開発期間の進捗遅れにより後回しにした機能
- [ ]  Service層リファクタリング
  - コーディング規約違反の修正と保守性向上（[Issue #9](https://github.com/SHUN5872698/learn-track-pro/issues/9)）
...
>
> [!WARNING]
> 各ドキュメントファイルにはGitHubリンクとNotionリンクを併記していますが、一部はNotionリンクのみのため、公開設定により閲覧できない場合があります。ご了承ください。
