<template>
  ...
  <div class="flex items-center justify-center min-h-screen px-4 py-12 bg-gray-50 sm:px-6 lg:px-8">
    ...
    <div class="z-10 w-full max-w-md p-10 space-y-8 bg-white shadow-lg rounded-xl">
      ...
      <!-- 成功メッセージ -->
      <div v-if="successMessage" class="p-4 mb-6 border-l-4 rounded-md text-emerald-800 bg-emerald-100 border-emerald-500">{{ successMessage }}</div>

      <!-- 認証エラー用（リセット失敗時） -->
      <div v-if="authError" class="p-4 mb-6 text-red-800 bg-red-100 border-l-4 border-red-500 rounded-md">{{ authError }}</div>
      ...
      <form class="mt-8 space-y-6" @submit.prevent="handlePasswordResetConfirm">
        ...
        <!-- 新しいパスワード入力フィールド -->
        <div>
          <label for="password" class="block text-sm font-medium text-gray-700">新しいパスワード<span class="pl-1 text-red-500">*</span></label>
...
// バリデーションルール
import { validateEmail, validatePassword, validatePasswordConfirmation } from '@/validators/profileValidator';
