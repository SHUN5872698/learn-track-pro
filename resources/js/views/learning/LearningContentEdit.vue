  ...
  try {
    const formDataWithStatus = {
      ...
      sections: form.sections.map((section, index) => {
        const originalSection = originalData.value.sections.find((s) => s.id === section.id);
        return {
          ...section,
          order: index + 1,
          status: originalSection ? originalSection.status : 'in_progress',
          completed_at: originalSection ? originalSection.completed_at : null,
        };
      }),
    };

    console.log('【Edit.handleSubmit】送信データ全体:', formDataWithStatus);
    console.log('【Edit.handleSubmit】セクション詳細:');
    formDataWithStatus.sections.forEach((section, index) => {
      const original = originalData.value.sections.find((s) => s.id === section.id);
      if (original) {
        console.log(`  ${index + 1}. ${section.title}: status = ${section.status} (既存・維持)`);
      } else {
        console.log(`  ${index + 1}. ${section.title}: status = ${section.status} (新規作成)`);
      }
    });

    await updateLearningContent(contentId, formDataWithStatus);
