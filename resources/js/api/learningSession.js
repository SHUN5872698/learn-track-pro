...
export const fetchSessionsBySection = (sectionId) => {
  return api.get(`${BASE_URL}/sections/${sectionId}/sessions`);
};

// Statistics
// 学習統計サマリーを取得するAPIリクエスト
export const fetchStatisticsSummary = () => {
  return api.get(`${BASE_URL}/learning-sessions/statistics/summary`);
};

// 月ごとの学習統計を取得するAPIリクエスト
export const fetchMonthlyStatistics = (months = 6) => {
  return api.get(`${BASE_URL}/learning-sessions/statistics/monthly`, { params: { months } });
};

// 技術ごとの学習統計を取得するAPIリクエスト
export const fetchTechnologyStatistics = () => {
  return api.get(`${BASE_URL}/learning-sessions/statistics/by-technology`);
};

// 日ごとの学習統計を取得するAPIリクエスト
export const fetchDailyStatistics = (days = 30) => {
  return api.get(`${BASE_URL}/learning-sessions/statistics/daily`, { params: { days } });
};
