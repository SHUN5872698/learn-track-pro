...
class LearningSessionController extends Controller
{
    ...
    public function statisticsSummary()
    {
        ...
        // 学習を行ったユニークな日付を取得
        $studyDays = $user->learningSessions()->selectRaw('DATE(studied_at) as date')
            ->distinct()
            ->pluck('date');
        ...
        if ($totalDays > 0) {
            // 日付を降順にソート
            $sortedDates = $studyDays->map(fn($date) => new \Carbon\Carbon($date))->sortDesc();
            $consecutiveDays = 1;
            $currentDate = $sortedDates->first();
            // ソートされた日付をループし、連続しているかを確認
            for ($i = 1; $i < $sortedDates->count(); $i++) {
                if ($currentDate->copy()->subDay()->isSameDay($sortedDates[$i])) {
                    $consecutiveDays++;
